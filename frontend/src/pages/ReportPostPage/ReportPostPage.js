...
function ReportPostPage() {
  ...
  const [buttonName, setButtonName] = useState("Ban Post");
  ...
  const getData = async () => {
    ...
    await axios
      .get(API_URL + "/complaintreview/getReportPostDetails/", config)
      .then((response) => {
        if (response.data.postDetails.blockedStatus) {
        ...
        setPostComplaint(response.data.postComplaint);
  ...
  const banUser = async () => {
    ...
    const inputData = {
      blockUserId: postDetails.postId,
    ...
    sendNotification(
      postDetails.creatorId,
      3,
      "We Have Banned Your Post Due to the Violation of Rules and Regulations of Post Sharing"
    );
        ...
        <div class="row justify-content-center">
          <div class="col-xl-7 col-lg-7 col-md-12 col-12 mb-5 ">
            <div class="card mt-2">
              ...
              <div class="card-body">
                <h5 class="card-title">
                  {typeList[postComplaint.reportCategory]}
                </h5>
                <p class="card-text">{postComplaint.description}</p>
                <button
